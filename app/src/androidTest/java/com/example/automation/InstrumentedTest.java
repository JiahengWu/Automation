...
// ./adb shell am instrument -w com.example.automation/android.support.test.runner.AndroidJUnitRunner;
/**
...
public class InstrumentedTest {
    ...
    private String date = dateFormatter.format(curDate);
    private String sdcardPath = Environment.getExternalStorageDirectory().getPath() + "/";

    // paths for log files
    private String time_log = "" + System.currentTimeMillis();
    private String m_logpathString = sdcardPath + date + "/" + time_log + "-activity.txt";
    private String m_logpathString_array = sdcardPath + date + "/" + time_log + "-array.txt";
    private String positionLog = sdcardPath + date + "/" + time_log + "-pos.txt";
    private String m_get_swipe_array = sdcardPath + "name.txt";
    ...
    public void startMainActivityFromHomeScreen() throws Exception {
        ...
        makeDirectory(sdcardPath + date);

        BufferedReader name = new BufferedReader(new InputStreamReader(new FileInputStream(m_get_swipe_array)));
        String fileName = name.readLine(); // file name for gestures
        String m_read_swipe_array = sdcardPath + fileName + ".txt";
        name.close();
    ...
    private void mlog(String str) {
        ...
        writeFile(m_logpathString, time + "," + str);
    }

    // mlog_2 function start
    private void mlog_2(String str, int id) {
        String time = "" + System.currentTimeMillis();
        writeFile(m_logpathString, time + "," + str + "," + id);
    }

    // log positions of each touch event
    private void logPos(long timestamp, int x, int y, String category) {
        writeFile(positionLog, timestamp + "," + x + "," + y + "," + category);
    }

    // mlog_array function start
    private void mlog_array(ArrayList list, Point[] point_array, int point_number) {
        String line = "";
        line += list.get(0);
        for (int i = 1; i < 4; i++) {
            line += "," + list.get(i);
        ...
        writeFile(m_logpathString_array, line);
    }
